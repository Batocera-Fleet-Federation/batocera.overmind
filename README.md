...
Overmind returns the current swarm list in the alive response, including each Drone's reported public IP and a public peer API URL derived from its advertised port and scheme. Each Drone stores that list, skips itself, and checks the other Drones through their public peer health API. The result says which Drone checked which peer, what address was used, whether it passed, how long it took, and the failure reason if it failed. Overmind stores those results and shows only the latest check per peer on the selected Drone page, using `RESOLVED` or `FAILED` labels.
