...
Overmind checks each Drone's globally routable public IP every 60 seconds by requesting `https://<public_ip>/health` on port 443 with a 1 second timeout. HTTP 200 marks that Drone Online; any other response, timeout, or connection failure marks it Offline and prevents that public peer URL from being advertised or queued as a ROM, BIOS, or artwork transfer source. The interval can be configured with `PUBLIC_PEER_PROBE_INTERVAL_SECONDS`; set it to `0` to disable probing. Each Drone stores the swarm list, skips itself, and checks resolvable peers through their public peer health API. Overmind stores those results and shows the latest check per peer on the selected Drone page.
