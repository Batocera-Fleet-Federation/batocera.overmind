...
Drones call Overmind every 30 seconds with an alive payload. That payload includes the MAC-address `device_id`, IPv4/IPv6 connectivity info, and ROM systems. Overmind validates the Drone bearer token, stores the latest network state, and only marks a Drone as `Connected to Swarm` when at least one reported IP resolves.
...
Drone tokens are generated by Overmind, stored as hashes, shown raw only once, and can be rotated from the Drone page. Demo mode uses `demo@example.com` and `demo-local-drone-token` so the local Drone can exercise alive updates, actions, logs, metadata, speed samples, and UI refresh.
