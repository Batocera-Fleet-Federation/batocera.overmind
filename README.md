...
Overmind periodically probes each Drone's globally routable public IP and advertised API port. A successful connection marks that Drone as resolvable and makes its derived public peer API URL available in the heartbeat swarm response; unverified or failed public endpoints are not advertised or queued as ROM, BIOS, or artwork transfer sources. The interval defaults to 60 seconds and can be configured with `PUBLIC_PEER_PROBE_INTERVAL_SECONDS`; set it to `0` to disable probing. Each Drone stores the swarm list, skips itself, and checks resolvable peers through their public peer health API. Overmind stores those results and shows the latest check per peer on the selected Drone page.
