...
Each Drone runs a background thread that periodically health-checks every other Drone in the swarm by making an mTLS request to `https://<public_ip>/health`. Drones without a public IP are skipped, and each Drone skips itself. Results are stored in the Drone's local SQLite database and reported to Overmind via the peer-checks API. Overmind derives each Drone's **Resolvable** status entirely from these peer reports: a Drone is resolvable if any other Drone has successfully reached its public endpoint. The Overmind UI shows outbound checks from the selected Drone and the list of Drones that have successfully resolved it. The check interval defaults to 300 seconds and can be adjusted with `DRONE_PEER_CHECK_INTERVAL_SECONDS`. Only peer-resolvable Drones are eligible as ROM, BIOS, and artwork transfer sources.
