...
- Local Compose uses PostgreSQL as the source of truth and shows unapproved Drone onboarding requests.
...
Overmind stores application state and collected Drone metadata in PostgreSQL. Configure one of:
...
The local Docker Compose file includes a lightweight `postgres:16-alpine` service for development data.

## Drone Push/Pull Architecture

Drones call Overmind every 30 seconds with an alive payload. That payload includes the MAC-address `device_id`, IPv4/IPv6 connectivity info, API port, protocol, certificate metadata, ROM systems, and system information. During each poll the Drone pushes only changed emulator configs and log-source content. Overmind validates the Drone bearer token, stores the latest network state, and updates `last_seen`.

Each Drone runs a background thread that periodically health-checks every other Drone in the swarm by making an mTLS request to `https://<public_ip>[:api_port]/health`. Drones without a public IP are skipped, and each Drone skips itself. Results are stored in the Drone's local SQLite database and reported to Overmind via the peer-checks API. Overmind derives each Drone's **Resolvable** status entirely from these peer reports: a Drone is resolvable if any other Drone has successfully reached its public endpoint. The Overmind UI shows outbound checks from the selected Drone and the list of Drones that have successfully resolved it. The check interval defaults to 300 seconds and can be adjusted with `DRONE_PEER_CHECK_INTERVAL_SECONDS`. Only peer-resolvable Drones are eligible as ROM, BIOS, and artwork transfer sources.
...
Drone tokens are generated by Overmind, stored as hashes, shown raw only once, and can be rotated from the Drone page.
...
The local swarm runs four lightweight Drone containers. Each has a unique device id, hostname, MAC address, port, and Docker volume.
